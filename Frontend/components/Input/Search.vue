<template>
  <div class="">
    <input
      id="search"
      type="text"
      name="search"
      class="w-full h-10 pl-10 pr-4 text-sm text-gray-400 placeholder-transparent border-2 border-gray-300 rounded-lg peer dark:bg-black dark:border-gray-800 sm:text-base focus:outline-none focus:border-gray-700 dark:focus:border-gray-700"
      placeholder="Search..."
    />
    <label
      for="search"
      class="absolute text-sm left-7 text-gray-600 transition-all px-1.5 dark:bg-black bg-white peer-placeholder-shown:left-10 -top-2 peer-placeholder-shown:text-base peer-placeholder-shown:text-gray-400 peer-placeholder-shown:top-2 peer-focus:left-7 peer-focus:-top-2 cursor-text peer-focus:text-gray-600 peer-focus:text-sm"
      >Search...</label
    >
  </div>
</template>
